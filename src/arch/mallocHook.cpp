...
#if defined(ARCH_OS_LINUX) && defined(__GLIBC__) && __GLIBC__ <= 2 \
    && __GLIBC_MINOR__ < 34
...
extern void* (*__MALLOC_HOOK_VOLATILE __malloc_hook)(
...
static inline bool _CheckMallocTagImpl(
    const std::string& impl, const char* libname)
{
    return (
        impl.empty() || impl == "auto" || impl == "agnostic"
...
static bool _MallocProvidedBySameLibraryAs(
    const char* functionName, bool skipMallocCheck)
{
#if !defined(ARCH_OS_WINDOWS)
...
{
    return (IsPxmallocActive() || IsPtmallocActive() || IsJemallocActive());
}
...
static _MallocFunctionNames _GetUnderlyingMallocFunctionNames()
{
...
bool IsPtmallocActive()
{
    ...
    return _MallocProvidedBySameLibraryAs(
        "__ptmalloc3_malloc", skipMallocCheck);
#else
    return false;
#endif
}

bool IsStlAllocatorOff()
{
...
bool MallocHook::Initialize(
    ...
    ARCH_UNUSED_ARG void (*freeWrapper)(void*, const void*), string* errMsg)
{
#if !defined(ARCH_OS_LINUX)
    *errMsg = "MallocHook only available for Linux/glibc systems";
    return false;
    ...
    *errMsg = "C library does not provide malloc hooks";
    ...
    if (IsInitialized()) {
        *errMsg = "MallocHook already initialized";
        return false;
    }

    if (!_MallocHookAvailable()) {
        *errMsg =
            "MallocHook functionality not available for current allocator";
        return false;
    }
