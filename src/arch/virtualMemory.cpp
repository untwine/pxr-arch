...
void *ReserveVirtualMemory(size_t numBytes)
{
    return VirtualAlloc(nullptr, numBytes, MEM_RESERVE, PAGE_NOACCESS);
}

bool CommitVirtualMemoryRange(void *start, size_t numBytes)
{
    return VirtualAlloc(start, numBytes, MEM_COMMIT, PAGE_READWRITE) != nullptr;
}

bool FreeVirtualMemory(void *start, size_t /*numBytes*/)
{
    ...
    void const *start, size_t numBytes, MemoryProtection protection)
{
    ...
    SIZE_T len = numBytes
                 + (reinterpret_cast<char const *>(start)
