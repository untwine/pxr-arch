...
///
ARCH_API
void LogSessionInfo(const char* crashStackTrace = nullptr);
...
ARCH_API
void PrintStackTrace(
    std::ostream& out, const std::string& programName,
...
ARCH_API
void LogStackTrace(
    const std::string& progName, const std::string& reason, bool fatal = false,
...
ARCH_API
void LogStackTrace(
    const std::string& reason, bool fatal = false,
    const std::string& sessionLog = "");
...
ARCH_API
void GetStackFrames(
    size_t maxDepth, size_t numFramesToSkipAtTop,
...
ARCH_API
size_t GetStackFrames(
    size_t maxDepth, size_t numFramesToSkipAtTop, uintptr_t* frames);

/// Print stack frames to the given ostream.
ARCH_API
void PrintStackFrames(
    std::ostream& out, const std::vector<uintptr_t>& frames,
