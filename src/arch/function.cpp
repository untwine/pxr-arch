...
string::size_type _GetStartOfName(const string& s, string::size_type i)
{
 ...
 *
 */
string _GetFunctionName(const string& function, const string& prettyFunction)
{
...
pair<string, string> _Split(const string& prettyFunction)
{
    auto i = prettyFunction.find(" [with ");
    if (i != string::npos) {
        const auto n = prettyFunction.size();
        return std::make_pair(
            prettyFunction.substr(0, i),
...
//   "A": "int", "B": "float"
// Note the leading space in the template list.
std::map<string, string> _GetTemplateList(const std::string& templates)
{
...
string _FormatTemplateList(const std::map<string, string>& templates)
{
    string result;
    if (!templates.empty()) {
 ...
 * Windows for the testFunction test.)
 */
string _GetNextIdentifier(const string& prettyFunction, string::size_type& pos)
{
...
// will be returned.
std::map<string, string> _FilterTemplateList(
    const string& prettyFunction, const std::map<string, string>& templates)
{
    std::map<string, string> result;
...
string GetPrettierFunctionName(
    const string& function, const string& prettyFunction)
{
