...
static size_t _ObtainCacheLineSize()
{
#if defined(ARCH_OS_LINUX)
    ...
    sysctlbyname(
        "hw.cachelinesize", &cacheLineSize, &cacheLineSizeSize, nullptr, 0);
...
void _ValidateAssumptions()
{
     ...
     */
    static_assert(
        sizeof(SomeEnum) == sizeof(int), "sizeof(enum) != sizeof(int)");
     ...
     */
    static_assert(
        sizeof(float) == sizeof(uint32_t) && sizeof(double) == sizeof(uint64_t)
