...
#define _ARCH_ENSURE_PER_LIB_INIT(T, prefix) \
    static _PerLibInit<T> _ARCH_CAT(prefix, __COUNTER__)
...
#define ARCH_CONSTRUCTOR(_name, _priority, ...)                                \
    ...
    static const _ConstructorEntry _ARCH_CAT_NOEXPAND(arch_ctor_, _name)       \
        __attribute__((used, section("__DATA,pxrctor"))) = {                   \
            reinterpret_cast<_ConstructorEntry::Type>(&_name), 0u, _priority}; \
    static void _name(__VA_ARGS__)

// Emit a _ConstructorEntry in the __Data,pxrdtor section.
#define ARCH_DESTRUCTOR(_name, _priority, ...)                                 \
    static void _name(__VA_ARGS__);                                            \
    static const _ConstructorEntry _ARCH_CAT_NOEXPAND(arch_dtor_, _name)       \
...
// Entry for a constructor/destructor in the custom section.
__declspec(align(16)) struct _ConstructorEntry {
...
#define ARCH_CONSTRUCTOR(_name, _priority, ...)                                \
    static void _name(__VA_ARGS__);                                            \
    namespace {                                                                \
    __declspec(allocate(".pxrctor")) extern const _ConstructorEntry            \
        _ARCH_CAT_NOEXPAND(arch_ctor_, _name) = {                              \
            reinterpret_cast<_ConstructorEntry::Type>(&_name), 0u, _priority}; \
    }                                                                          \
    _ARCH_ENSURE_PER_LIB_INIT(_ConstructorInit, _archCtorInit);                \
    static void _name(__VA_ARGS__)
...
#define ARCH_DESTRUCTOR(_name, _priority, ...)                                 \
    static void _name(__VA_ARGS__);                                            \
    namespace {                                                                \
    __declspec(allocate(".pxrdtor")) extern const _ConstructorEntry            \
