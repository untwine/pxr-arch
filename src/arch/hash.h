...
///
ARCH_API uint32_t Hash(const char *data, size_t len);
/// \overload
ARCH_API uint32_t Hash(const char *data, size_t len, uint32_t seed);
...
/// contiguous pieces as a whole.  Support for that may be added in future.
///
ARCH_API uint64_t Hash64(const char *data, size_t len);
/// \overload
ARCH_API uint64_t Hash64(const char *data, size_t len, uint64_t seed);
