    ...
    if (is_64) {
        for (size_t i = 0; i != nsects; ++i) {
            ...
            if (strcmp(section->sectname, sectname) == 0) {
                return {
                    static_cast<intptr_t>(section->addr) + _slide,
...
std::vector<_ConstructorEntry> GetConstructorEntries(
    const struct mach_header* mh, intptr_t slide, const char* segname,
    const char* sectname)
{
...
void RemoveImage(const struct mach_header* mh, intptr_t slide)
{
...
static std::vector<_ConstructorEntry> GetConstructorEntries(
    HMODULE hModule, const char* sectname)
{
    std::vector<_ConstructorEntry> result;
