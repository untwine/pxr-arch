...
static void _FixupStringNames(string* name)
{
    ...
    static auto* to = new string("string");
...
{
    int status;
    if (char* realName = abi::__cxa_demangle(
            mangledTypeName->c_str(), nullptr, nullptr, &status)) {
...
static string* _NewDemangledStringTypeName()
{
    auto* result = new string(typeid(string).name());
    _DemangleNewRaw(result);
    return result;
}

bool Demangle(string* mangledTypeName)
{
    ...
    if (_DemangleNew(mangledTypeName)) {
        if (_DemangleOld(&copy) && copy != *mangledTypeName) {
            fprintf(
                stderr,
                "Demangle: disagreement between old and new\n"
...
void _DemangleFunctionName(string* mangledFunctionName)
{
...
string GetDemangled(const string& typeName)
{
    string r = typeName;
    if (Demangle(&r)) return r;
    return {};
}

string GetDemangled(const char* typeName)
{
    if (typeName) {
        string r = typeName;
        if (Demangle(&r)) {
