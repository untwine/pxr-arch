...
#if defined(ARCH_OS_WINDOWS)
    return _wfopen(
        WindowsUtf8ToUtf16(fileName).c_str(), WindowsUtf8ToUtf16(mode).c_str());
#else
...
{
    return RemoveDirectoryW(WindowsUtf8ToUtf16(path).c_str()) ? 0 : -1;
}
#endif

bool StatIsWritable(const StatType* st)
{
...
bool GetModificationTime(const char* pathname, double* time)
{
    StatType st;
#if defined(ARCH_OS_WINDOWS)
    if (_wstat64(WindowsUtf8ToUtf16(pathname).c_str(), &st) == 0)
#else
    if (stat(pathname, &st) == 0)
#endif
    {
        *time = GetModificationTime(st);
        return true;
    }
    return false;
}

double GetModificationTime(const StatType& st)
{
...
namespace {  // Helpers for NormPath.
...
string _NormPath(string const& inPath)
{
    ...
    for (; t.first != inPath.end(); t = _NextToken(t.second, inPath.end())) {
        switch (_GetTokenType(t)) {
            case Elem:
                // Copy the elem.  We avoid mutating 'path' if we've made no
                // changes to the output yet, which is true if the write head is
                // in the same place in the output as it is in the input.
            ...
            case DotDot: {
                ...
                // Find the last token of the output by finding the next token
                // in reverse.
                RToken backToken = _NextToken(rwrite, rstart);
                // If there are no more Elems to consume with DotDots and this
                // is a relative path, or this token is already a DotDot, then
                ...
                if ((rstart == path.rend() && backToken.first == rstart)
                    ...
                    // Otherwise, consume the last elem by moving writeIdx back
                    // to before the elem.
...
#if defined(ARCH_OS_WINDOWS)
string NormPath(const string& inPath, bool stripDriveSpecifier)
{
    ...
    //
    // Also make sure drive letters are always lower-case out of NormPath
...
string AbsPath(const string& path)
{
    ...
    if (GetFullPathNameW(
            WindowsUtf8ToUtf16(path).c_str(), ARCH_PATH_MAX, buffer, nullptr)) {
        return WindowsUtf16ToUtf8(buffer);
    }
    else {
        return path;
    }
    ...
    if (getcwd(cwd.get(), ARCH_PATH_MAX) == nullptr) {
        return path;
    }

    return NormPath(string(cwd.get()) + "/" + path);
#endif
}

bool GetStatMode(const char* pathname, int* mode)
{
    StatType st;
#if defined(ARCH_OS_WINDOWS)
...
double GetAccessTime(const struct stat& st)
{
#if defined(ARCH_OS_LINUX)
...
double GetStatusChangeTime(const struct stat& st)
{
#if defined(ARCH_OS_LINUX)
...
int64_t GetFileLength(FILE* file)
{
    if (!file) return -1;
#if defined(ARCH_OS_LINUX) || defined(ARCH_OS_DARWIN)
    struct stat buf {};
...
int64_t GetFileLength(const char* fileName)
{
    ...
    HANDLE handle = CreateFileW(
        WindowsUtf8ToUtf16(fileName).c_str(),
        0,
...
string GetFileName(FILE* file)
{
    ...
    char buf[PATH_MAX];
    ssize_t r = readlink(
        StringPrintf("/proc/self/fd/%d", fileno(file)).c_str(),
        buf,
        sizeof(buf));
    if (r != -1) {
...
string MakeTmpFileName(const string& prefix, const string& suffix)
{
    ...
    if (n == 1)
        return StringPrintf(
            "%s/%s.%d%s", tmpDir.c_str(), prefix.c_str(), pid, suffix.c_str());
    else
        return StringPrintf(
            "%s/%s.%d.%d%s",
            tmpDir.c_str(),
...
int MakeTmpFile(const std::string& prefix, std::string* pathname)
{
    return MakeTmpFile(GetTmpDir(), prefix, pathname);
}

#if defined(ARCH_OS_WINDOWS)

namespace {
std::string MakeUnique(
    const std::string& sTemplate, std::function<bool(const char* name)> func,
    int maxRetry = 1000)
{
...
int MakeTmpFile(
    const std::string& tmpdir, const std::string& prefix, std::string* pathname)
{
    // Format the template.
    std::string sTemplate =
        StringPrintf("%s/%s.XXXXXX", tmpdir.c_str(), prefix.c_str());
...
std::string MakeTmpSubdir(const std::string& tmpdir, const std::string& prefix)
{
    ...
    retstr = MakeUnique(sTemplate, [](const char* name) {
        return CreateDirectoryW(WindowsUtf8ToUtf16(name).c_str(), nullptr)
               != FALSE;
    });
#else
    // Copy template to a writable buffer.
...
static const char* _TmpDir = nullptr;
...
void _InitTmpDir()
{
    ...
    _TmpDir = _strdup(WindowsUtf16ToUtf8(tmpPath).c_str());
#else
    const std::string tmpdir = GetEnv("TMPDIR");
    if (!tmpdir.empty()) {
        // This function is not exposed in the header; it is only used during
        // _InitConfig. If this is called more than once when TMPDIR is
...
void _Unmapper::operator()(char const* mapStart) const
{
...
void _Unmapper::operator()(char* mapStart) const
{
    (*this)(static_cast<char const*>(mapStart));
}

template <class Mapping>
static inline Mapping _MapFileImpl(FILE* file, std::string* errMsg)
{
    using PtrType = typename Mapping::pointer;
    constexpr bool isConst =
        std::is_const<typename Mapping::element_type>::value;

    auto length = GetFileLength(file);
    ...
    HANDLE hFileMap = CreateFileMapping(
        _FileToWinHANDLE(file),
        nullptr,
        PAGE_READONLY /* allow read-only or copy-on-write */,
        maxSizeHigh,
        maxSizeLow,
        nullptr);
    if (hFileMap == nullptr) return Mapping();
    auto ptr = static_cast<PtrType>(MapViewOfFile(
        hFileMap,
    ...
    return Mapping(ptr, _Unmapper(length));
#else  // Assume POSIX
    auto m = mmap(
        nullptr,
        length,
        ...
        0);
    Mapping ret(
        m == MAP_FAILED ? nullptr : static_cast<PtrType>(m), _Unmapper(length));
    if (!ret && errMsg) {
        int err = errno;
        if (err == EINVAL) {
...
ConstFileMapping MapFileReadOnly(FILE* file, std::string* errMsg)
{
    return _MapFileImpl<ConstFileMapping>(file, errMsg);
}

MutableFileMapping MapFileReadWrite(FILE* file, std::string* errMsg)
{
    return _MapFileImpl<MutableFileMapping>(file, errMsg);
}
...
template <class Mapping>
static inline Mapping _MapFileImpl(std::string const& path, std::string* errMsg)
{
    _UniqueFILE f(OpenFile(path.c_str(), "rb"));
    if (!f) {
        if (errMsg) {
            *errMsg = StrError();
        }
    ...
    return _MapFileImpl<Mapping>(f.get(), errMsg);
}

ConstFileMapping MapFileReadOnly(std::string const& path, std::string* errMsg)
{
    return _MapFileImpl<ConstFileMapping>(path, errMsg);
}

MutableFileMapping MapFileReadWrite(
    std::string const& path, std::string* errMsg)
{
    return _MapFileImpl<MutableFileMapping>(path, errMsg);
}
...
void MemAdvise(void const* addr, size_t len, MemAdvice adv)
{
    ...
    // This must follow MemAdvice exactly.
    int adviceMap[] = {
        /* MemAdviceNormal       = */ POSIX_MADV_NORMAL,
        /* MemAdviceWillNeed     = */ POSIX_MADV_WILLNEED,
        /* MemAdviceDontNeed     = */ POSIX_MADV_DONTNEED,
        /* MemAdviceRandomAccess = */ POSIX_MADV_RANDOM};

    int rval = posix_madvise(
        reinterpret_cast<void*>(alignedAddrInt),
    ...
    void const* addr, size_t len, unsigned char* pageMap)
{
    ...
    int ret = mincore(
        reinterpret_cast<caddr_t>(const_cast<void*>(addr)),
        len,
...
int64_t PRead(FILE* file, void* buffer, size_t count, int64_t offset)
{
    ...
    DWORD numRead = 0;
    if (ReadFile(
            hFile, buffer, static_cast<DWORD>(count), &numRead, &overlapped)) {
        return numRead;
    }
    return -1;
#else  // assume POSIX
...
static inline DWORD ModeToAccess(int mode)
{
    switch (mode) {
        case X_OK:
...
int FileAccess(const char* path, int mode)
{
    ...
    std::wstring wpath{WindowsUtf8ToUtf16(path)};
    if (mode == F_OK) {
    ...
    DWORD length = 0;
    if (!GetFileSecurityW(wpath.c_str(), securityInfo, nullptr, 0, &length)) {
        if (GetLastError() != ERROR_INSUFFICIENT_BUFFER) {
            return _FileAccessError();
        }
    }
    ...
    HANDLE token;
    DWORD desiredAccess = TOKEN_IMPERSONATE | TOKEN_QUERY | TOKEN_DUPLICATE
    ...
    if (DuplicateToken(token, SecurityImpersonation, &duplicateToken)) {
        ...
        DWORD accessMask = ModeToAccess(mode);
...
#else  // assume POSIX
    // This must follow FileAdvice exactly.
    int adviceMap[] = {
        /* FileAdviceNormal       = */ POSIX_FADV_NORMAL,
        /* FileAdviceWillNeed     = */ POSIX_FADV_WILLNEED,
        /* FileAdviceDontNeed     = */ POSIX_FADV_DONTNEED,
        /* FileAdviceRandomAccess = */ POSIX_FADV_RANDOM};
    int rval = posix_fadvise(
        fileno(file), offset, static_cast<off_t>(count), adviceMap[adv]);
    if (rval != 0) {
        fprintf(
            stderr,
