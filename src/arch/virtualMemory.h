...
/// Reserve \p numBytes bytes of virtual memory.  Call CommitVirtualMemory()
/// on subranges to write to and read from the memory.  Return nullptr in case
/// of an error; check errno.
ARCH_API void *ReserveVirtualMemory(size_t numBytes);
...
/// ReserveVirtualMemory().  It is not an error to commit a range that was
/// previously partly or fully committed.  Return false in case of an error;
/// check errno.
ARCH_API bool CommitVirtualMemoryRange(void *start, size_t numBytes);

/// Return memory obtained with ReserveVirtualMemory() to the system.  The
/// \p start argument must be the value returned from a previous call to
/// ReserveVirtualMemory, and \p numBytes must match the argument from that
/// call.  Memory within the range may not be accessed after this call.  Return
/// false in case of an error; check errno.
ARCH_API bool FreeVirtualMemory(void *start, size_t numBytes);

/// Memory protection options, see SetMemoryProtection().
enum MemoryProtection {
    ProtectNoAccess,
    ProtectReadOnly,
    ProtectReadWrite,
    ProtectReadWriteCopy
};
...
/// ProtectReadWrite and ProtectReadWriteCopy are the same, on Windows
...
ARCH_API bool SetMemoryProtection(
    void const *start, size_t numBytes, MemoryProtection protection);

}  // namespace arch

#endif  // ARCH_VIRTUAL_MEMORY_H
