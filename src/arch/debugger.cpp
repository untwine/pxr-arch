...
static char** _archDebuggerAttachArgs = nullptr;

#if defined(ARCH_OS_LINUX) || defined(ARCH_OS_DARWIN)
static void _DebuggerTrapHandler(int)
{
...
#if defined(ARCH_OS_LINUX) || defined(ARCH_OS_DARWIN)
static void _DebuggerInitPosix()
{
...
// or return.
bool _DebuggerRunUnrelatedProcessPosix(bool (*cb)(void*), void* data)
{
    ...
    //
    // Note: BSD systems use ioctl(fd, TIOCNOTTY, nullptr) to disable
...
static bool _DebuggerAttachExecPosix(void* data)
{
    char** args = (char**)data;
...
static bool _DebuggerIsAttachedPosix()
{
...
static bool _DebuggerAttach()
{
    ...
    //   ARCH_DEBUGGER='osascript -e "tell application \"Terminal\"" -e
    //   "activate" -e "set newTab to do script(\"lldb -p %p\")" -e "end tell"'
...
void _InitDebuggerAttach()
{
    ...
    if (e && e[0]) {
        ...
        _archDebuggerAttachArgs[3] = nullptr;
...
void DebuggerWait(bool wait) { _archDebuggerWait = wait; }

namespace {
bool _AvoidJIT() { return (getenv("ARCH_AVOID_JIT") != nullptr); }
}  // namespace

bool DebuggerAttach()
{
    return !_AvoidJIT() && (DebuggerIsAttached() || _DebuggerAttach());
}
