...
{
    return _Replace(
        _Replace(_Replace(std::move(pattern), ".", "\\."), "*", ".*"),
        "?",
        ".");
}
    ...
    const std::string& pattern, unsigned int flags, std::string* error)
{
    ...
    if (flags & Regex::CASE_INSENSITIVE) {
...
bool Regex::_Impl::Match(const char* query) const
{
    std::cmatch result;
    ...
    const std::string& pattern, unsigned int flags, std::string* error)
{
    const int regflags = REG_EXTENDED | REG_NEWLINE
                         | ((flags & Regex::CASE_INSENSITIVE) ? REG_ICASE : 0);
...
Regex::_Impl::~_Impl() { regfree(&_regex); }

bool Regex::_Impl::Match(const char* query) const
{
    return regexec(&_regex, query, 0, nullptr, 0) != REG_NOMATCH;
}

#endif  // defined(ARCH_OS_WINDOWS)

Regex::Regex(const std::string& pattern, unsigned int flags) : _flags(flags)
{
    try {
        if (pattern.empty()) {
...
Regex::Regex(Regex&& rhs) noexcept
...
Regex::operator bool() const { return static_cast<bool>(_impl); }

std::string Regex::GetError() const
{
    return _impl ? "" : (_error.empty() ? "uncompiled pattern" : _error);
}

unsigned int Regex::GetFlags() const { return _flags; }

bool Regex::Match(const std::string& query) const
{
