...
{
    // Non-member
    ARCH_AXIOM(arch::GetPrettierFunctionName("Bar", "int Bar(float)")
             == "Bar");
 
    // Template non-member function
    ARCH_AXIOM(arch::GetPrettierFunctionName("Bar", "int Bar(C) [with C = int]")
             == "Bar");
    ARCH_AXIOM(arch::GetPrettierFunctionName("Bar", "int Bar<C>(C)")
             == "Bar");
 
    // Non-template class non-template member function
    ARCH_AXIOM(arch::GetPrettierFunctionName("Bar", "int Foo::Bar(float)")
             == "Foo::Bar");
 
    // Template class member function
    ARCH_AXIOM(arch::GetPrettierFunctionName("Bar",
    ...
    ARCH_AXIOM(arch::GetPrettierFunctionName("foo",
    ...
    // Edge cases.
    ARCH_AXIOM(arch::GetPrettierFunctionName("operator<", "bool operator<(X, Y)")
             == "operator<");
    ARCH_AXIOM(arch::GetPrettierFunctionName("operator<", "bool Z<W>::operator<(Y) const [with W = int]")
             == "Z<W>::operator< [with W = int]");
    ARCH_AXIOM(arch::GetPrettierFunctionName("operator<<", "int operator<<(X, int)")
             == "operator<<");
