...
TEST(FileSystemTest, FileOperations)
{
    ...
    ASSERT_TRUE(std::filesystem::equivalent(ArchWindowsUtf8ToUtf16(filePath),
                   ArchWindowsUtf8ToUtf16(firstName)));
#else
    ASSERT_TRUE(std::filesystem::equivalent(filePath, firstName));
#endif
    fclose(firstFile);

    // Test utf-8 path
    std::string secondName = ArchMakeTmpFileName("测试");
    ASSERT_NE((firstFile = ArchOpenFile(secondName.c_str(), "w")), nullptr);
    filePath = ArchGetFileName(firstFile);
    ASSERT_TRUE(std::filesystem::equivalent(std::filesystem::u8path(filePath),
