...
TEST(DemangleTest, TemplatedClassRecursiveInt)
{
    ...
    const char* const oldStyle = "MangledTemplatedClass<MangledTemplatedClass<int> >";
    const char* const newStyle = "MangledTemplatedClass<MangledTemplatedClass<int>>";

    const auto isDemangledCorrectly = [&](const std::string& demangledName) {
        return demangledName == oldStyle || demangledName == newStyle;
    };

    ASSERT_TRUE(isDemangledCorrectly(toBeDemangledName));
    ASSERT_TRUE(isDemangledCorrectly(ArchGetDemangled(mangledName)));
    ASSERT_TRUE(isDemangledCorrectly(ArchGetDemangled(typeInfo)));
    ASSERT_TRUE(isDemangledCorrectly(
