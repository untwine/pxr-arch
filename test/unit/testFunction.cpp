...
{
    // Non-member
    ASSERT_EQ(ArchGetPrettierFunctionName("Bar", "int Bar(float)"), "Bar");

    // Template non-member function
    ASSERT_EQ(ArchGetPrettierFunctionName("Bar", "int Bar(C) [with C = int]"),
        "Bar");
    ASSERT_EQ(ArchGetPrettierFunctionName("Bar", "int Bar<C>(C)"), "Bar");

    // Non-template class non-template member function
    ASSERT_EQ(ArchGetPrettierFunctionName("Bar", "int Foo::Bar(float)"),
        "Foo::Bar");
    ...
    // Edge cases.
    ASSERT_EQ(
        ArchGetPrettierFunctionName("operator<", "bool operator<(X, Y)"),
        "operator<");
    ASSERT_EQ(
        ArchGetPrettierFunctionName(
    ...
    ASSERT_EQ(
        ArchGetPrettierFunctionName("operator<<", "int operator<<(X, int)"),
        "operator<<");
}
